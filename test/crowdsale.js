...
const RefundVault = artifacts.require('./zeppelin/RefundVault.sol')
...
contract('PowerPiperCrowdsale', function ([owner, wallet, investor, otherInvestor]) {
  ...
  beforeEach('setup crowdsale contract for each test', async function () {
    ...
    this.crowdsale = await PowerPiperCrowdsale.new(
      this.openingTime, this.closingTime, rate, cap, wallet, this.token.address
    )
  ...
  it('should fail with zero cap', async function () {
    await PowerPiperCrowdsale.new(rate, wallet, 0, this.token.address).should.be.rejectedWith(EVMRevert)
  })
